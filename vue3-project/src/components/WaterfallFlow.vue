...
const distributeContent = (newItems = []) => {
  ...
  // 使用 requestAnimationFrame 延迟布局计算，并增加节流
...
// 监听预加载笔记数据变化
watch(
  () => props.preloadedPosts,
  async (newPosts, oldPosts) => {
    if (
    ...
    if (
      ...
      const isSameData = newPosts.every(
        (post, index) => oldPosts[index] && post.id === oldPosts[index].id
      );
      if (isSameData) {
...
<template>
  ...
  <div v-else ref="containerRef" class="waterfall-container">
    <div v-if="contentList.length === 0 && !loadingMore" class="empty-state">
      <div class="empty-text">
        <template v-if="props.type === 'posts'"> 还没有发布任何内容 </template>
        ...
        <template v-else-if="props.searchKeyword"> 没有找到相关内容 </template>
    ...
    <div v-else class="waterfall-columns" :style="{ gap: columnGap + 'px' }">
      ...
      >
        ...
        >
          <BaseSkeleton
            v-if="!isItemFullyLoaded(item.id)"
            type="image-card"
            image-height="random"
            :show-stats="false"
            :show-button="false"
          ...
          <div
            class="item-content"
            :class="{ 'content-hidden': !isItemFullyLoaded(item.id) }"
          >
            ...
            <div class="contentlist">
              <img
                v-img-lazy="item.avatar"
                alt=""
                class="lazy-avatar clickable-avatar"
                @error="handleAvatarError"
                @load="onImageLoaded(item.id, 'avatarLoaded')"
                @click="onUserClick(item.author_account, $event)"
              />
              <div
                class="contentlist-name clickable-name"
                @click="onUserClick(item.author_account, $event)"
              >
                {{ item.author }}
              </div>
              <div class="action-wrapper">
                <div class="like-num-wrapper">
                  <LikeButton
                    :is-liked="likeStore.getPostLikeState(item.id).liked"
                    @click="
                      (willBeLiked, event) =>
                        onLikeClick(item, willBeLiked, event)
                    "
                  />
                  <span class="like-num">{{
                    likeStore.getPostLikeState(item.id).likeCount
                  }}</span>
                </div>
              </div>
            </div>
    ...
    <div
      ...
      :class="{ 'no-more-content': !hasMore && contentList.length > 0 }"
    >
    ...
    <!-- 添加底部缓冲区，防止滚动边界问题 -->
    <div v-if="!hasMore && contentList.length > 0" class="bottom-buffer"></div>
  </div>
  ...
  <Teleport to="body">
    <DetailCard
      ...
      @follow="handleDetailCardFollow"
      @unfollow="handleDetailCardUnfollow"
      @like="handleDetailCardLike"
