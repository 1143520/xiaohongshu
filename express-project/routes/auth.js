...
router.post('/register', async (req, res) => {
  try {
    ...
    if (user_id.length < 3 || user_id.length > 15) {
      return res.status(400).json({ code: 400, message: '小红书号长度必须在3-15位之间' });
    }

    if (!/^[a-zA-Z0-9_]+$/.test(user_id)) {
      return res.status(400).json({ code: 400, message: '小红书号只能包含字母、数字和下划线' });
    }
    ...
    console.log(`用户注册成功 - 用户ID: ${userId}, 小红书号: ${userRows[0].user_id}`);
...
router.post('/login', async (req, res) => {
  try {
    ...
    console.log(`用户登录成功 - 用户ID: ${user.id}, 小红书号: ${user.user_id}`);
